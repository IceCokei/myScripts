...
function ParseResponse() {
    try {
        if ($response && $response.body) {
            ...
            const mobile = data.data && data.data.mobile; 
            ...
            if (memberId && mobile && token && projectId) {
                const newCookie = `${mobile}#${token}#${projectId}#${memberId}`; 
                if ($persistentStore.write(newCookie, cookieName)) {
                    $notification.post("健达福利社", "", "✅ Cookie和id、memberId获取/更新成功！");
                } else {
                    $notification.post("健达福利社", "", "❌ Cookie写入失败，请重试！");
                }
            } else {
                $notification.post("健达福利社", "", "❌ 未找到有效的token、projectId、memberId或mobile，请重试！");
            }
        }
    } catch (e) {
